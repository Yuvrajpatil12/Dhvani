...
{
    public class VMStyleListBusinessFacade//:UniversalObject
    {
        dynamic objdynamicWrapper; 
		VMStyleListWrapperColletion objVMStyleListWrapperColletion = new VMStyleListWrapperColletion();
        VMStyleListWrapper objVMStyleListWrapper = new VMStyleListWrapper();
		private static readonly string _module = "Core.Business.BusinessFacade.VMStyleListBusinessFacade";
		public VMStyleListBusinessFacade()
        {
            
        }
        public VMStyleListBusinessFacade(dynamic WrapperType)
        {
        ...
        public dynamic GetVMStyleListRecordById(string voiceMasterID)
        {
            ...
            try
            {
                _getList = objVMStyleListWrapperColletion.GetVMStyleListRecordById(voiceMasterID);
            }
            catch (Exception ex)
            {
                Log.WriteLog(_module, $"GetVMStyleListRecordById({voiceMasterID})", ex.Source, ex.Message, ex);
            }
