...
function SubmitLogin() {    
    ...
    else if ($("#terms-of-use").prop("checked") == false) {
        $("#divLoginMsg > div.alert").remove();
        $("#divLoginMsg").prepend(getAlert("The terms-of-use must be accepted.", "danger", false));
    }
    else {
...
function DoLogin(username, password, queryString, isRemember, autologin) {
    $("#divLoginLoader").append(getLoader());
    $.ajax({
        url: '../Users/Login',
        ...
        success: function (data) {
            ...
            if (data.isSuccess) {
                ...
                if (data.returnUrl != null && data.returnUrl != undefined) {
                    if (data.data.roleId == 1 || data.data.roleId == 2 || data.data.roleId == 3) {
